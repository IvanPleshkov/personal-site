---
title: "TurboQuant + RaBitQ: a hybrid approach in Qdrant"
description: "How I shipped a TurboQuant + RaBitQ hybrid in Qdrant 1.18 with additional tricks."
pubDate: 2026-04-29
...
Important context up front. TurboQuant is not the only algorithm in the "random rotation + per-coordinate scalar quantization" family. In parallel — and earlier, at SIGMOD 2024 — there's **[RaBitQ (Gao & Long)](https://arxiv.org/abs/2405.12497)**, built on the same foundation but with different implementation details and different answers to the key engineering problems, in particular shrinkage. My implementation in Qdrant is a **hybrid** of the two: I take the Lloyd-Max codebook and the integer SIMD scoring kernels (at 2 and 4 bits) from TurboQuant; length renormalization and the entire 1-bit scoring path from RaBitQ — at 1 bit the algorithm reduces to pure RaBitQ, with TurboQuant contributing only the rotation. On top of that hybrid I add one substantive product-level extension — **per-coordinate anisotropy compensation** — and a handful of engineering details that make the whole stack land in production (a pure reversible Hadamard rotation, P-Square calibration, a wide query for 1-bit, support for L2 and unnormalized dot). The body of the post calls out, section by section, where each piece comes from.

Some tricks I picked up from open-source implementations in the community — most notably [llama-turbo-quant](https://github.com/llama-research/llama-turbo-quant) and [Arkar Min Aung's interactive explainer](https://arkaung.github.io/interactive-turboquant/) — and they dropped cleanly onto Qdrant's setup. I'm not pretending I invented all of this — but I'll try to explain **why** each of these extensions fits Qdrant as a production-grade vector database.
...
The simplest of our tricks, and the one that overdelivers the most on impact. Up front: **I didn't invent this, and it isn't in the TurboQuant paper either**. It's an idea from [RaBitQ (Gao & Long, SIGMOD 2024)](https://arxiv.org/abs/2405.12497) — an algorithm from the same "rotation + quantization" family that pre-dates TurboQuant and solves the same shrinkage problem not via QJL sampling but via one extra scalar per vector. The open-source TurboQuant implementations (notably [llama-turbo-quant](https://github.com/llama-research/llama-turbo-quant)) borrowed the trick from RaBitQ; I in turn first saw it there. The most accurate way to think of it is **TurboQuant's codebook with RaBitQ's scale on top**.

Not crediting it would be dishonest — it's the trick that closes most of the gap between vanilla TurboQuant and our implementation.
...
> **Important note:** in this table `+ renormalization` means our implementation **with only the renormalization trick**, none of the other tricks from this post. The pure isolated effect of layering the RaBitQ trick on top of vanilla TurboQuant. Our full implementation (with all the tricks) does even better — its numbers are in the [main recall table](#recall) above.
...
First, the context. At 1-bit storage the codebook is just two points `±c`, where `c = sqrt(2/π) ≈ 0.798` (Lloyd-Max for N(0, 1)) — meaning the codebook index *is* the sign bit of the stored coordinate. The storage layout and the scoring kernel at this bit width I take directly from [**RaBitQ**](https://arxiv.org/abs/2405.12497), unmodified: on the data side it's a dense pack of signs (8 dims per byte, bit-plane layout at 128 dims per 16-byte chunk); the query is quantized to `B` bits per coordinate and transposed into `B` bit-planes — one plane per query bit. In that layout the dot product collapses into one `AND`+popcount per plane, weighted by `2^b`. This is **RaBitQ's direct contribution to Qdrant's 1-bit path** — the TurboQuant paper itself adds nothing 1-bit-specific beyond the Lloyd-Max codebook with two centroids, and the fast implementation of that case comes from RaBitQ.

The specific SIMD machinery (how `AND`+popcount maps to AVX-512/NEON instructions) is the topic of the separate post on SIMD optimizations; here all that matters is one thing: **the query bit width `B` is a free parameter, which we can dial without changing the storage format at all.**
...
TurboQuant is a beautiful algorithm, and the MSE variant in particular fits a vector database really well. But between "the algorithm from the paper" and "a production-grade implementation in Qdrant" there's a gap — and the honest way to describe what closes it is as a **hybrid of TurboQuant and [RaBitQ (Gao & Long, SIGMOD 2024)](https://arxiv.org/abs/2405.12497)**, with one product-level extension of my own on top.

**The hybrid foundation.** This is where most of the recall actually lives:

- **Lloyd-Max codebook + 2/4-bit integer SIMD scoring** — from TurboQuant.
- **Length renormalization via a stored centroid-norm** — from RaBitQ. Closes the shrinkage gap without QJL; alone it delivers +9–15pp recall on anisotropic datasets at 4 bits.
- **The entire 1-bit path** (dense pack of signs + bit-plane `AND`+popcount scoring) — from RaBitQ. At 1 bit the algorithm reduces to pure RaBitQ; TurboQuant doesn't contribute anything 1-bit-specific.

**My one product-impacting extension.** Per-coordinate **anisotropy compensation** via `(shift, scale)` offloaded onto the query side — free at compute, but real points of recall on anisotropic embeddings, and identity-by-construction on isotropic ones (so it never makes things worse). This is the only thing in the post that, by itself, moves the recall curve on real production data.

**Engineering polish that makes the hybrid land in production but doesn't move recall on its own.** Worth mentioning so the post stands on its own, but I want to be honest that none of these are product-level innovations:

- A **pure reversible Hadamard rotation** built from LCG + Fisher-Yates permutations — chosen to keep zero rotation metadata on disk. A nicer engineering solution to a problem any rotation-based quantizer faces.
- **P-Square calibration** — the mechanism through which anisotropy compensation gets its `(shift, scale)` without assuming a Gaussian distribution.
- **Wide query for 1-bit with compensation** — a small fix for a rounding edge case.
- **L2 and unnormalized dot** off the unit sphere, at the cost of `+4` bytes per vector for the stored length. L1 doesn't yield to extension (rotation doesn't preserve L1) — an honest limitation; for L1 don't use TQ in production.
